...
import styles from "./ChooseSendMultiple.module.css";
...
export const ChooseSendMultiple: React.FC<AppProps> = (props) => {
    ...
    Office.context.mailbox.getCallbackTokenAsync((result) => {
      ...
      httpClient.getAttachments(serviceRequest).then((responseData) => {
        if (responseData.length == 1) {
          props.navigatorFunction(["enterDataMultiple", responseData[0].fileBase64, responseData[0].name]);
        } else {
              ...
              {attachments.map((attachment, index) => {
                return (
                  <div
                    ...
                    onClick={() => {
                      props.navigatorFunction(["enterDataMultiple", attachment.fileBase64, attachment.name]);
                    }}
                  >
...
export default ChooseSendMultiple;
